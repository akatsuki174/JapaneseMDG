...
多くの場合、8dp以上のスペースで仕切られたタッチターゲットだと、バランスの取れた情報密度とユーザビリティが作られます。
...
アクセシビリティテキストとは、Androidに搭載されているGoogleのTalkBack、iOSに搭載されているAppleのVoiceOver、デスクトップに搭載されているFreedom ScientificのJAWSなど、アクセシビリティソフトウェアのスクリーンリーダーで使われるテキストのことを指します。スクリーンリーダーは画面上のテキストやボタンなどの要素を声に出して読み上げますが、これには目に見えるものと目に見えない代替テキストの両方が含まれます。

#### 目に見えるテキスト、目に見えないテキスト

アクセシビリティテキストには目に見えるテキスト（ボタンやリンク、フォームのテキストなどのUI要素に付与されたラベルも含む）と、画面に表示されない説明（画像の代替テキストなど）の両方が含まれます。より多くの情報をユーザに伝えるために、画面上のラベルがアクセシビリティテキストに上書きされることもあります。

目に見えるテキストも目に見えないテキストが説明的で意味を成していれば、それはユーザが画面上の見出しやリンクを使って移動するのに役立ちます。スクリーンリーダーはアクセシビリティテキストをテストし、まだ追加できる場所を発見するのに役立ちます。

#### 代替テキスト

代替テキストは視覚的なUIをテキストベースのUIに変換するのに役立ちます。代替テキストとは、画像を見ることができないユーザのために画像を説明した、コード上の短いラベル（最大125文字）のことです。代替テキストは画像のみを対象にしているため、代替テキストに「画像の」「図の」という言葉を追加する必要はありません。スクリーンリーダーが画像の代わりに代替テキストを声に出して読んでくれます。画像の読み込みが失敗したときにも表示される代替テキストは、視覚障害のあるユーザにとって価値があります。画像についての情報を伝えるために、ターゲットを絞ったキーワードを含めるようにしましょう。キーワードは検索エンジン最適化（SEO）を向上させることができます。

[スクショ]

良い例：画像が何を示しているのかを、情報に富んだ短いフレーズで伝えられるように代替テキストを使用しています。

代替テキストの例：The Tokyo Tower and skyline at night
